...
const CastMember: React.FC<CastMemberProps> = ({
  ...
  const characterName =
    "character" in person ? person.character : role || person.job;
    ...
    >
      ...
      <div className="flex flex-col items-center space-y-2">
        ...
        <div className="relative">
          <motion.div
            className="w-20 h-20 rounded-full overflow-hidden border-2 border-white/20 bg-gradient-to-br from-gray-700 to-gray-900 flex items-center justify-center"
          ...
          <AnimatePresence>
            {isHovered && (
              ...
              >
                <motion.button
                  className="bg-galaxy-purple hover:bg-galaxy-red text-white px-2 py-1 rounded-full text-xs font-medium"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  <svg
        ...
        <div className="text-center">
          ...
          {showRole && characterName && characterName !== person.name && (
            <p className="text-gray-400 text-xs leading-tight">
              {characterName.length > 15
                ? `${characterName.substring(0, 12)}...`
                : characterName}
            </p>
          )}
        </div>
